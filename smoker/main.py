...
wc=["WELCOME TO CVVVerifiedBot‼️🥷🏽\nHere at CVVVerifiedBot you will find EVERYTHING you need to succeed‼️\n\n🤝Here you can automatically buy what you need🤫\n🤝If you have any problems - our support is online 24/7!\n🤝Good Luck‼️"]
hel=["admin will update soon"]
fa=["admin will update soon"]
te=["admin will update soon"]
phoy=["AgACAgQAAxkBAAIBTWI1QAY9KgKCOO9oCiywksCV44QHAAJ8uTEb1HyoUXS_OrHhev4bAQADAgADeQADIwQ"]
BUTTON,AB,DES,ABAD,DESAD,CATEGORY,CATEGORYAD,PR,PRAD,PD,PID,CART,DELETE,PHO,PHOAD,MODIFY,PIDAD,CH,SH,WA,SHO,JE,ADDRESS,STOCK,PHOSTOCK,DESSTOCK,PIDSTOCK,PRSTOCK,CATEGORYSTOCK,PDSTOCK,HAM,AMAN,RMAN,AMO,TRC,CRP,FCAT,TYPP,MODF,MODFF,AKEY,COU,COD,DEL,FAQ,C1,C2,C3,C4,ADFILE,ADFILEE,CNN,TMS= range(53) 
man=[]
def start(update, context):
  ...
  if len(jobs) ==0:
      ...
      conn.execute("INSERT INTO COMPANY (ID,pid,pname,quantity,amount) \
              VALUES ('{}', '{}','{}', '{}','{}')".format(str(update.effective_user.id),"0","0","0","0")) 
      conn.commit()

  user = update.message.from_user
  usa=str(update.effective_user.id)
  if usa == "6320891323" or usa == "1394902938" or usa =="993953033" or usa in man:
    keyboard =[[InlineKeyboardButton("➕ Product", callback_data="1"),InlineKeyboardButton("❌ Product", callback_data="2"),InlineKeyboardButton("⚙️ Modify Product", callback_data="32")],[InlineKeyboardButton("⚙️ Delete Category", callback_data="del"),InlineKeyboardButton("⚙️ ADD Category", callback_data="90")],[InlineKeyboardButton("🛒 Orders", callback_data="99"),InlineKeyboardButton("🛒 ADD File", callback_data="1file"),InlineKeyboardButton("Managers List", callback_data="lman")],[InlineKeyboardButton("🙋‍♂️ Welome message", callback_data="912"),InlineKeyboardButton("❓ Set Help / Support", callback_data="913"),InlineKeyboardButton("Edit FAQ", callback_data="faq")],[InlineKeyboardButton("🔊 Announcement", callback_data="916"),InlineKeyboardButton("Create Transcation", callback_data="trc")],[InlineKeyboardButton("Add manager", callback_data="aman"),InlineKeyboardButton("Remove Manager", callback_data="rman")],[InlineKeyboardButton("Change Photo", callback_data="3678"),InlineKeyboardButton("Create Coupon", callback_data="cp")],[InlineKeyboardButton("Update terms and conditions", callback_data="tms")]]
    ...
    keyf=[["📝️ Products","🛒 Cart"],["💳 Wallet","💳 Add Credit"],["🛍 My orders","❓ Help/Support"],["🙋 FAQ","Terms and Conditions"]]
    ...
    if a=="3344":
      ...
      if a==1:
        ...
        if qp=="Physical":
          ...
          file_path = "{}.txt".format(pid) 
          lines = []
          with open(file_path, "r") as file:
              lines = file.readlines()
          a=len(lines) 
          conn = sqlite3.connect("oo.db")  
          conn.execute("UPDATE COMPANY set amount = '{}', pid='{}',pname='{}',quantity='{}' where ID = {}".format(pric,pid,nam,1,int(update.effective_user.id)))
          conn.commit()
          keyboard =[[InlineKeyboardButton("🔙 cancel", callback_data="200")]]
          reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
          context.bot.delete_message(chat_id=update.effective_user.id,message_id=cf)
          context.bot.send_message(chat_id=update.effective_user.id,text="How much quantity you want to buy max quantity is {}".format(a),reply_markup=reply_markup)
          return CNN
    elif a=='2801':
            query.answer()
            ...
            if summl>inv:
              ...
              reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
              context.bot.send_doucement(chat_id=update.effective_user.id,text="Select the Payment option\nTotal amount= "+str(summl)+"$",reply_markup=reply_markup)
              return BUTTON
    elif a=="2902":
            query.answer()
            userg = update.callback_query.message.chat
            usaf=userg.first_name
            conn = sqlite3.connect('oo.db')
            cursor = conn.execute("SELECT pid,amount,pname,quantity from COMPANY where ID= '{}'".format(str(update.effective_user.id)))
            conn.commit()
            for row in cursor:
                x=row[0]
                suma=int(row[3])
                file_path = "{}.txt".format(x) 
                lines = []
                with open(file_path, "r") as file:
                    lines = file.readlines()
                a=len(lines) 
                ...
                if a>=bq:
                      ...
                      for row in cursor:
                          ...
                          if inv<tu:
                            context.bot.send_message(chat_id=update.effective_user.id,text="Your Balance is not enough")
                              ...
                              if a <5:
                                context.bot.send_message(chat_id=admin_id,text="low quantity for prouct ID = {}".format(x))
                              first_line=""
                              for mey in range(0,scu):
                                first_line =first_line + lines.pop(0) +"\n"
                                
                                with open(file_path, "w") as file:
                                    file.writelines(lines)
                              with open("{}.txt".format(update.effective_user.id), "w") as file:
                              ...
                              for row in cursor:
                                  ...
                                  conn.execute("INSERT INTO COMPANY (ID,name,price,oid,address,date,quantity,pnam,productID,status) \
                                          VALUES ('{}', '{}','{}', '{}', '{}', '{}', '{}','{}','{}','{}')".format(str(update.effective_user.id),usaf,row[1],str(v),"none",dat,row[3],row[2],row[0],"pending")) 
                                  conn.commit()
                                  conn = sqlite3.connect('orders.db')
                                  conn.execute("INSERT INTO COMPANY (ID,name,price,oid,address,date,quantity,pnam,productID,status) \
                                          VALUES ('{}', '{}','{}', '{}', '{}', '{}', '{}','{}','{}','{}')".format(str(update.effective_user.id),usaf,row[1],str(v),"none",dat,row[3],row[2],row[0],"pending")) 
                              ...
                              keyboard =[[InlineKeyboardButton("🔙 Back", callback_data="200")]]
                              reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
                              context.bot.send_document(chat_id=update.effective_user.id,document=open('{}.txt'.format(update.effective_user.id), 'rb'),reply_markup=reply_markup)
                              return BUTTON
                  ...
                  if bq==0:
                     context.bot.send_message(chat_id=admin_id,text='Product {} is out of stock'.format(x))
                     
                  keyboard =[[InlineKeyboardButton("🔙 cancel", callback_data="200")]]
                  reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
                  context.bot.send_message(chat_id=update.effective_user.id,text='Quantity of product is not availbe right now please try agai'.format(a),reply_markup=reply_markup)
                  return CNN
      ...
      keyboard =[[InlineKeyboardButton("🔙 Back", callback_data="100")]]
      reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
      context.bot.send_message(chat_id=update.effective_user.id,text='Send your terms and conditions to update',reply_markup=reply_markup)
      return TMS
      ...
      context.bot.send_message(chat_id=update.effective_user.id,text="Please send the screenshot of your payment\n\n")
      return COU
      ...
      return HAM   
        ...
        hjam="Quantity:  {}".format(xdd)
        hjamn="Quantity:  {}".format(dy)
        cy=cy.replace(hjam,hjamn)
      
        cy=cy.replace("{}$".format(fgh),"{}$".format(np))
        keyboard =[[InlineKeyboardButton("➕ Add", callback_data="16"),InlineKeyboardButton("🔙 Cancel", callback_data="200")],[InlineKeyboardButton("+ Quantity", callback_data="160"),InlineKeyboardButton("- Quantity", callback_data="161")]]
        reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
        context.bot.edit_message_caption(chat_id=update.effective_user.id,message_id=cf,caption=cy,reply_markup=reply_markup)              
    elif a== '120': #Accept
        c=update.callback_query.message.message_id
        cc=update.callback_query.message.text
        d=cc
...
def men(update,context):
    ...
    if msg=="📝️ Products":
      ...
      return BUTTON
        ...
        keyboard =[[InlineKeyboardButton("Cash App",callback_data="cash")],[InlineKeyboardButton("Coupon", callback_data="cou")],[InlineKeyboardButton("Crypto", callback_data="9956")]]
        reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
      ...
      reply_markup = ReplyKeyboardMarkup(keyf,resize_keyboard=True)
      context.bot.send_message(chat_id=update.effective_user.id,text=wc[0],reply_markup=reply_markup)
    elif msg == "Terms and Conditions":
       context.bot.send_message(chat_id=update.effective_user.id,text=te[0])
...
def adfilee(update,context):
    ...
    filef.download('{}.txt'.format(eopl))
...
def cnn(update,context):
  ...
  try:
    ...
    if suma>0:
      conn = sqlite3.connect('oo.db')
      cursor = conn.execute("SELECT pid,amount,pname,quantity from COMPANY where ID= '{}'".format(update.effective_user.id))
      conn.commit()
      for row in cursor:
          print(row)
          x=row[0]
          ...
          if a>=suma:
              ...
              if ac>inv:
                ...
                conn = sqlite3.connect("oo.db")  
                conn.execute("UPDATE COMPANY set quantity = '{}' where ID = {}".format(suma,update.effective_user.id))
                conn.commit()
                conn.close()
                keyboard =[[InlineKeyboardButton("Pay from Wallet", callback_data="2902")]]
                reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
                context.bot.send_message(chat_id=update.effective_user.id,text="Select the Payment option\nTotal amount= "+str(ac)+"$",reply_markup=reply_markup)
                return BUTTON
            
          
          else:
            if a==0:
               context.bot.send_message(chat_id=update.effective_user.id,text='Product {}is out of stock'.format(bq))
            
            keyboard =[[InlineKeyboardButton("🔙 cancel", callback_data="200")]]
            reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
            context.bot.send_message(chat_id=update.effective_user.id,text='qunantity exceed max you can buy is {}'.format(a),reply_markup=reply_markup)
            return CNN
    
    else: 
      keyboard =[[InlineKeyboardButton("🔙 Back", callback_data="200")]]
      reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
      context.bot.send_message(chat_id=update.effective_user.id,text='Invalid qunantity!',reply_markup=reply_markup)
      return CNN
  except:
    keyboard =[[InlineKeyboardButton("🔙 Back", callback_data="200")]]
    reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
...
def faq(update,context): 
       ...
       context.bot.send_message(chat_id=update.effective_user.id,text="Help/Support updated",reply_markup=reply_markup)
       return BUTTON
def tms(update,context): 
       a=update.message.text
       te.pop(0)
       te.append(a)
       keyboard =[[InlineKeyboardButton("🔙 Backk", callback_data="100")]]
       reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
       context.bot.send_message(chat_id=update.effective_user.id,text="terms and conditions updated updated",reply_markup=reply_markup)
       return BUTTON
...
def pd(update,context):
    ...
    for names in cursor:
      ...
      if inv in msg:
        ...
        for row in cursor:
          file_path = "{}.txt".format(row[3]) 
          lines = []
          with open(file_path, "r") as file:
              lines = file.readlines()
          a=len(lines)   
          if a==0:
            context.bot.send_message(chat_id=update.effective_user.id,text='Product {}is out of stock'.format(row[3]))
            m="Product name:  "+row[0]+"\nProduct ID:  "+row[3]+"\nPrice:  "+row[2]+"$"+"\nDescripton:  "+row[1]+"\nProduct Type:  "+row[6] +"\nTotal Quantity:  {}".format(a)
            keyboard =[[InlineKeyboardButton("🔙 Out of Stock", callback_data="200")]]
            reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
            context.bot.send_message(chat_id=admin_id,text='Product {} is out of stock'.format(row[3]))
            context.bot.send_photo(chat_id=update.effective_user.id,photo=row[4],caption=m,reply_markup=reply_markup)

          else:
                                  
            m="Product name:  "+row[0]+"\nProduct ID:  "+row[3]+"\nPrice:  "+row[2]+"$"+"\nDescripton:  "+row[1]+"\nProduct Type:  "+row[6] +"\nTotal Quantity:  {}".format(a)
            keyboard =[[InlineKeyboardButton("➕ Add", callback_data="16"),InlineKeyboardButton("🔙 Cancel", callback_data="200")]]
            reply_markup = InlineKeyboardMarkup(keyboard,resize_keyboard=True,one_time_keyboard=True)
            context.bot.send_photo(chat_id=update.effective_user.id,photo=row[4],caption=m,reply_markup=reply_markup)
        break
...
def main():
  ...
  conv_handler = ConversationHandler(
        entry_points=[CommandHandler("start", start),MessageHandler(Filters.regex('^(📝️ Products|🛒 Cart|💳 Wallet|💳 Add Credit|🛍 My orders|❓ Help/Support|🔙 Main Menu|🤝 Affiliate Program|🙋 FAQ|Terms and Conditions)$'), men),CallbackQueryHandler(button)],
        ...
        states={
            ...
            CNN: [MessageHandler(Filters.text, cnn),CallbackQueryHandler(button)],
            ...
            TMS: [MessageHandler(Filters.text, tms),CallbackQueryHandler(button)],
